  ...
  return (
    <div className="tasks">
      <div className="show-one-task">
        {editTaskId === showTheTask.id ? (
          <input
            className="edit-input"
            type="text"
            value={editTaskTitle}
            onChange={handleEditTaskChange}
            onBlur={() => saveEditedTask(showTheTask.id)}
            onKeyDown={(e) => {
              if (e.key === "Enter") {
                if (editTaskTitle.trim() === "") {
                  saveEditedTask(showTheTask.title);
                } else {
                  saveEditedTask(showTheTask.id);
                }
              }
            }}
          />
          ...
          <>
            <p>{showTheTask.title}</p>
            <div className="controls">
              <button
                className="delete-button"
                onClick={() => deleteTask(showTheTask.id)}
              >
                <svg height="17.5" width="15" className="icon">
                  <path
              ...
              <MdOutlineEdit
                className="edit"
                onClick={() =>
                  startEditingTask(showTheTask.id, showTheTask.title)
                }
              />
              <div
                className="check"
